"""Monta o pacote portatil do DocFlow Manager e, se pedido, publica no GitHub.

Uso tipico:

    py -3 publicar_release.py --versao 8.1.0 --notas NOTAS_RELEASE_8.1.0.md --publicar

Sem ``--publicar`` o script so gera ``dist/DocFlowManager-<versao>.zip`` e o
arquivo ``.sha256`` ao lado, para conferencia ou instalacao manual.
"""
...
from atualizador import APP_FILES, APP_ID, MANIFEST, PACKAGE_PREFIX, UpdateError, read_version, sha256, version_tuple, write_json
...
def build_package(root: Path, version: str | None = None, output: Path | None = None) -> Path:
    ...
    archive = output / f"{PACKAGE_PREFIX}-{version}.zip"
...
def api(token: str, url: str, method="GET", data=None, binary=False):
    ...
    request = urllib.request.Request(url, data=body, method=method, headers={
        "Authorization": "Bearer " + token, "User-Agent": "DocFlowManager-Publisher",
...
def publish_package(root: Path, archive: Path, create_repository=False, notes="") -> str:
    ...
    try:
        ...
        repo = api(token, "https://api.github.com/user/repos", "POST", {
            "name": name, "description": "Distribuicao e atualizacoes do DocFlow Manager",
    ...
    release = api(token, base + "/releases", "POST", {
        "tag_name": "v" + info["version"], "name": "DocFlow Manager " + info["version"],
        "draft": True, "prerelease": False, "target_commitish": repo["default_branch"],
        "body": "Pacote portatil do DocFlow Manager para Windows (Python 3.11+, pywebview e pypdf).\n\n"
                "Na primeira instalacao, extraia o ZIP e execute instalar_bibliotecas.bat. "
                "Abra por DocFlow_Manager.pyw. As proximas versoes sao verificadas ao abrir o programa.\n\n"
                "A uniao de DWG precisa do AutoCAD instalado."
                + ("\n\n" + notes if notes else ""),
    })
    upload = release["upload_url"].split("{")[0]
    # O release fica como rascunho ate o pacote e a soma de verificacao subirem.
...
def main():
    ...
    parser.add_argument("--versao", help="Nova versao, por exemplo 8.1.0")
